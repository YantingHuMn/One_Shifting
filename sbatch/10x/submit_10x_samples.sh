...
tail -n +2 "$tsv" | awk -v n="$run_first_n_sample" 'NR <= n {print $2}' | while read -r sample_name; do
    ...
    train_job_id=$(sbatch \
        ...
        --error="${log_dir}/submit_10x_${sample_name}_%a.err" \
    ...
    plot_job_id=$(sbatch \
        --job-name="10x_plot_${sample_name}" \
        --dependency=afterok:${train_job_id} \
        --exclude=compute-170 \
        --export=ALL,sample_name="${sample_name}" \
        --output="${log_dir}/plot_10x_${sample_name}_%A.out" \
        --error="${log_dir}/plot_10x_${sample_name}_%A.err" \
