load_rna_atac_from_h5ad <- function(path, out_dir, upstream = 2000) {
    ...
    dir.create(file.path(out_dir, "G120_RNA"), recursive = TRUE, showWarnings = FALSE)
    dir.create(file.path(out_dir, "G120_ATAC"), recursive = TRUE, showWarnings = FALSE)
    ...
    write_feather(rna_counts_df, file.path(out_dir, "G120_RNA", "rna_counts.feather"))
    write_feather(activity_counts_df, file.path(out_dir, "G120_ATAC", "activity_counts.feather"))
...
path <- args[1]
out_dir <- args[2]
load_rna_atac_from_h5ad(path, out_dir)