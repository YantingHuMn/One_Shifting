...
transpose <- if (length(args) >= 6 && !is.na(args[6]) && args[6] != "") args[6] else FALSE
...
run_build_norm_filter_pipeline(path1 = path1, path2 = path2, out_dir = out_dir, norm_factor1 = norm_factor1, norm_factor2 = norm_factor2, filtered_percentile = 0.25, lib_size_min = lib_size_min, transpose = transpose)
