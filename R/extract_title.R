...
extract_title <- function(filename) {
    ...
    # New filename format:
    # d_pearson_col_scatter_v1_trans_count+1_norm_1000_v2_trans_no_trans_norm_no_norm
    # d_pearson_v1_reverse_col_scatter_v1_trans_count+1_norm_1000_v2_trans_no_trans_norm_no_norm
    if (grepl("_scatter_v1_trans_", name_without_ext) &&
        grepl("_v2_trans_", name_without_ext)) {

        v1_trans <- sub("^.*_scatter_v1_trans_(.*)_norm_.*_v2_trans_.*$", "\\1", name_without_ext)
        v1_norm  <- sub("^.*_scatter_v1_trans_.*_norm_(.*)_v2_trans_.*$", "\\1", name_without_ext)
