...
def filter_and_transform(df1, df2, threshold_value, trans1, trans2, transformed_out_dir, data_path1=None, data_path2=None, save=False):
    ...
    if save:
        ...
        if data_path1 is not None:
            save_path1 = transformed_out_dir / f"Count_matrix_transformed_rep1.feather"
            filtered_df1.to_feather(save_path1)
            print(f"[SAVE] Saved transformed df1 ({trans1}) to: {save_path1}")
        
        if data_path2 is not None:
            save_path2 = transformed_out_dir / f"Count_matrix_transformed_rep2.feather"
...
def main(args):
    ...
    # --- Locate model weights and config by extension ---
    # Try final_model first (files directly in saved_models_dir with "final" in name)
    final_weights = None
    final_config = None
    for ext in ("*.pt", "*.pth"):
    ...
    if 'trans1' in cfg and 'trans2' in cfg:
        trans1 = cfg['trans1']
        trans2 = cfg['trans2']
        print(f"  trans1 (V1): {trans1}")
        print(f"  trans2 (V2): {trans2}")
    elif 'trans' in cfg:
        trans1 = trans2 = cfg['trans']
        print(f"  trans (both): {trans1}")
        print("  [WARNING] Using legacy single transformation for both V1 and V2")
    else:
        raise ValueError("Configuration must contain either 'trans' or both 'trans1' and 'trans2'")
    ...
    print(f"       Used V1 transformation: {trans1}")

if __name__ == "__main__":
    ap = argparse.ArgumentParser()
    ap.add_argument("--data_path1", required=True, help="First feather（One need reconstruction）")
