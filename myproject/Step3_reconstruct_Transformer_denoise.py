...
def filter_and_transform(df1, df2, threshold_value, trans1, trans2,
    ...
    filtered_df2 = filtered_df2.rename(columns={first_col_name: 'pos'})
    ...
    if save and transformed_out_dir is not None:
        ...
        if data_path2 is not None:
            save_path2 = transformed_out_dir / "Count_matrix_transformed_rep2.feather"
...
def main(args):
    ...
    print(f"       Used V2 transformation: {trans2}")


if __name__ == "__main__":
    ap = argparse.ArgumentParser(description="Transformer Autoencoder Reconstruct: denoise V1 data")
    ap.add_argument("--data_path1", required=True, help="V1 feather file (to denoise)")
    ap.add_argument("--data_path2", required=True, help="V2 feather file (for column filtering)")
