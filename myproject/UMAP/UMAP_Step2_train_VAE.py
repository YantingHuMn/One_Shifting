...
def vae_loss(recon_x, x, mu, logvar, beta, weight_strategy='fixed', zero_weight=1.0, nonzero_weight=5.0, trans=None):
    recon_loss = weighted_reconstruction_loss(recon_x, x, weight_strategy, zero_weight, nonzero_weight, trans)
...
def filter_and_transform(df1, threshold_value, trans1, data_path1=None, save=False):
    ...
    if save and data_path1 is not None:
...
def _prepare_tensors(filtered_df1):
    ...
    if not all(df1.dtypes.apply(lambda x: np.issubdtype(x, np.number))):
...
def outer10_inner_holdout(
    ...
    if has_negatives_df1:
        ...
        print(f"[INFO] Negative values detected, filtered trans1_grid to: {trans1_grid}")

    # --- Pre-compute all (threshold, trans1) combinations ---
    transform_cache = {}
    tensor_cache = {}
    for threshold, trans1 in product(threshold_grid, trans1_grid):
        ...
        if cache_key not in transform_cache:
            ...
            transform_cache[cache_key] = fdf1
            tensor_cache[cache_key] = _prepare_tensors(fdf1)
    ...
    for fold_id, (outer_train_idx, outer_test_idx) in enumerate(kf.split(df1_raw), 1):
        ...
        for threshold, trans1, hidden_dim1, hidden_dim2, latent_dim, lr, bs, beta, zero_w, nonzero_w in product(
            threshold_grid, trans1_grid, hidden_grid1, hidden_grid2, latent_grid, lr_grid, bs_grid, beta_grid,
            ...
            config_name = f"{zero_w}_{nonzero_w}_{trans1}_{threshold}_{hidden_dim1}_{hidden_dim2}_{latent_dim}_{beta}"
            ...
            if val_loss < best_val:
                best_val = val_loss
                best_cfg = dict(threshold=threshold, trans1=trans1,
        ...
        print(f"[Fold {fold_id}] Best validation config: {best_cfg}, val_loss={best_val:.4f}")

        fold_best_cfgs.append(best_cfg)
        fold_val_losses.append(best_val)

        # Retrain best config on full training set, evaluate on test
...
def main(args):
    ...
    if args.out_summary:
        ...
        with open(args.out_summary, "w") as f:
            ...
            f.write("fold\tthreshold\ttrans1\thidden_dim1\thidden_dim2\tlatent_dim\tlr\tbatch_size\tbeta\tzero_weight\tnonzero_weight\tinner_val_loss\ttest_loss\n")
            for i, (cfg, vl, tl) in enumerate(zip(
                results["best_cfgs_per_fold"],
                results["val_losses_per_fold"],
                results["test_losses_per_fold"]
            ), 1):
                f.write(f"{i}\t{cfg['threshold']}\t{cfg['trans1']}\t{cfg['hidden_dim1']}\t{cfg['hidden_dim2']}\t{cfg['latent_dim']}\t{cfg['lr']}\t{cfg['batch_size']}\t{cfg['beta']}\t{cfg['zero_weight']}\t{cfg['nonzero_weight']}\t{vl:.6f}\t{tl:.6f}\n")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="VAE with Weighted Reconstruction Loss - Single Input")
    parser.add_argument('--data_path1', type=str, required=True, help='Path to feather file')
