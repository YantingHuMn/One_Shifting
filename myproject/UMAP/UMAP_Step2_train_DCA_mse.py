...
def main(args):
    ...
    if args.out_summary:
        ...
        with open(args.out_summary, "w") as f:
            ...
            f.write("fold\tthreshold\ttrans1\thidden_dim1\thidden_dim2\tlatent_dim\tlr\tbatch_size\tdropout\tzero_weight\tnonzero_weight\tinner_val_loss\ttest_loss\n")
            for i, (cfg, vl, tl) in enumerate(zip(
                results["best_cfgs_per_fold"],
                results["val_losses_per_fold"],
                results["test_losses_per_fold"]
            ), 1):
                f.write(f"{i}\t{cfg['threshold']}\t{cfg['trans1']}\t{cfg['hidden_dim1']}\t{cfg['hidden_dim2']}\t{cfg['latent_dim']}\t{cfg['lr']}\t{cfg['batch_size']}\t{cfg['dropout']}\t{cfg['zero_weight']}\t{cfg['nonzero_weight']}\t{vl:.6f}\t{tl:.6f}\n")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="DCA (Deep Count Autoencoder) - Single Input")
