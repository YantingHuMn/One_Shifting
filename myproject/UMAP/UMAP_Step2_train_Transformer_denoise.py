...
def _prepare_tensors(filtered_df1):
    ...
    for col in df1.columns:
        if df1[col].dtype == 'object':
            print(f"[WARNING] Converting object column {col} in df1")
            df1[col] = pd.to_numeric(df1[col], errors='coerce')
    df1.fillna(0, inplace=True)

    if not all(df1.dtypes.apply(lambda x: np.issubdtype(x, np.number))):
        raise ValueError("Cannot convert to tensor: non-numeric data present in df1")
...
def outer_cv_inner_holdout(
    ...
    for fold_id, (outer_train_idx, outer_test_idx) in enumerate(kf.split(df1_raw), 1):
        ...
        for threshold, trans1, n_tokens, d_model, nhead, num_layers, dim_ff, dropout, lr, bs, zero_w, nonzero_w in product(
            threshold_grid, trans1_grid, n_tokens_grid, d_model_grid, nhead_grid, num_layers_grid,
...
def main(args):
    ...
    if args.out_summary:
        ...
        with open(args.out_summary, "w") as f:
            ...
            f.write("fold\tthreshold\ttrans1\tn_tokens\td_model\tnhead\tnum_layers\tdim_feedforward\tdropout\tlr\tbatch_size\tzero_weight\tnonzero_weight\tinner_val_loss\tinner_val_metric\ttest_loss\ttest_metric\n")
            for i, (cfg, vl, vm, tl, tm) in enumerate(zip(
                ...
                f.write(f"{i}\t{cfg['threshold']}\t{cfg['trans1']}\t{cfg['n_tokens']}\t{cfg['d_model']}\t{cfg['nhead']}\t{cfg['num_layers']}\t{cfg['dim_feedforward']}\t{cfg['dropout']}\t{cfg['lr']}\t{cfg['batch_size']}\t{cfg['zero_weight']}\t{cfg['nonzero_weight']}\t{vl:.6f}\t{vm:.6f}\t{tl:.6f}\t{tm:.6f}\n")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Transformer Autoencoder: V1 -> V1 denoising (single input)")
