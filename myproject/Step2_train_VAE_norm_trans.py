...
def eval_correlation_residual(model, loader, v2_data, indices, device, corr_type='pearson'):
    ...
    for col_idx in range(n_cols):
        ...
        if np.any(np.isnan(vae_col)) or np.std(vae_col) == 0:
            ...
            if corr_type == 'pearson':
                corr_vae_v2, _ = pearsonr(vae_col, v2_col)
            elif corr_type == 'spearman':
...
def _apply_norm(df, norm):
    """
    Apply library-size normalization to df (first column is pos/barcode, rest are data).
    Matches the R build_and_normalize logic:
      - "no_norm": no change
      - "standardize": column-wise standardization (z-score)
      - numeric string (e.g. "10000"): library-size normalization to that total
    """
    if norm == "no_norm":
        return
    ...
    if norm == "standardize":
        ...
        data = data.div(lib_sizes, axis=1) * norm_factor
    
    df.iloc[:, 1:] = data


def filter_and_transform(df1, df2, threshold_value, trans1, trans2, norm1="no_norm", norm2="no_norm",
    ...
    # Apply normalization before transformation
    _apply_norm(filtered_df1, norm1)
    _apply_norm(filtered_df2, norm2)
...
def outer10_inner_holdout(
    ...
    threshold_grid, trans1_grid, trans2_grid, norm1_grid, norm2_grid,
    ...
    # --- Pre-compute all (threshold, trans1, trans2, norm1, norm2) combinations ---
    transform_cache = {}
    tensor_cache = {}
    for threshold, trans1, trans2, norm1, norm2 in product(threshold_grid, trans1_grid, trans2_grid, norm1_grid, norm2_grid):
        cache_key = (threshold, trans1, trans2, norm1, norm2)
        if cache_key not in transform_cache:
            # Skip incompatible combinations: standardize can produce negatives
            if norm1 == "standardize" and trans1 in {'sqrt', 'sqrt+1', 'sqrt+0.00001', 'sqrt+10',
                                                      'sqrt+1_then_minus_1', 'log2', 'log2_then_add_1',
                                                      'log2(count+2)', 'log2(count+1)+1', 'log(count+2)'}:
                print(f"[SKIP] norm1=standardize incompatible with trans1={trans1}")
                continue
            if norm2 == "standardize" and trans2 in {'sqrt', 'sqrt+1', 'sqrt+0.00001', 'sqrt+10',
                                                      'sqrt+1_then_minus_1', 'log2', 'log2_then_add_1',
                                                      'log2(count+2)', 'log2(count+1)+1', 'log(count+2)'}:
                print(f"[SKIP] norm2=standardize incompatible with trans2={trans2}")
                continue
            fdf1, fdf2 = filter_and_transform(df1_raw, df2_raw, threshold, trans1, trans2, norm1, norm2)
            transform_cache[cache_key] = (fdf1, fdf2)
            tensor_cache[cache_key] = _prepare_tensors(fdf1, fdf2)
    print(f"[CACHE] Pre-computed {len(transform_cache)} (threshold, trans1, trans2, norm1, norm2) combinations.\n")
    ...
    for fold_id, (outer_train_idx, outer_test_idx) in enumerate(kf.split(df1_raw), 1):
        ...
        for threshold, trans1, trans2, norm1, norm2, hidden_dim1, hidden_dim2, latent_dim, lr, bs, beta, zero_w, nonzero_w in product(
            threshold_grid, trans1_grid, trans2_grid, norm1_grid, norm2_grid,
            hidden_grid1, hidden_grid2, latent_grid, lr_grid, bs_grid, beta_grid, 
            ...
            if cache_key not in tensor_cache:
                continue  # skipped incompatible combination
            ...
            for ep in range(1, epochs_inner + 1):
                ...
                if early_stop and (ep % check_every == 0):
                    if eval_metric == 'val_loss':
                        curr_val = eval_loss(model, val_loader, device, beta, weight_strategy, zero_w, nonzero_w, trans1)
            ...
            if eval_metric == 'val_loss':
                val_metric = eval_loss(model, val_loader, device, beta, weight_strategy, zero_w, nonzero_w, trans1)
                val_loss = val_metric
            elif eval_metric in ['pearson', 'spearman']:
                val_metric = eval_correlation_residual(model, val_loader, X2, val_idx, device, eval_metric)
                val_loss = eval_loss(model, val_loader, device, beta, weight_strategy, zero_w, nonzero_w, trans1)
            else:
                raise ValueError(f"Unknown eval_metric: {eval_metric}")
            
            config_name = f"{norm1}_{norm2}_{zero_w}_{nonzero_w}_{trans1}_{trans2}_{threshold}_{hidden_dim1}_{hidden_dim2}_{latent_dim}_{beta}"
        ...
        norm1 = best_cfg["norm1"]
        norm2 = best_cfg["norm2"]
        ...
        with open(os.path.join(fold_dir, "vae_config.json"), "w") as f:
            json.dump({
                ...
                "norm1": str(best_cfg["norm1"]),
                "norm2": str(best_cfg["norm2"]),
...
def main(args):
    ...
    norm1_grid = [x.strip() for x in args.norm1_grid.split(",") if x.strip() != ""]
    norm2_grid = [x.strip() for x in args.norm2_grid.split(",") if x.strip() != ""]

    # When using val_loss, V2 is not involved in evaluation, so fix trans2/norm2 to avoid redundant runs
    if args.eval_metric == 'val_loss':
        if len(trans2_grid) > 1 or len(norm2_grid) > 1:
            print(f"[INFO] eval_metric=val_loss: V2 not used in evaluation. "
                  f"Collapsing trans2_grid={trans2_grid} -> ['{trans2_grid[0]}'], "
                  f"norm2_grid={norm2_grid} -> ['{norm2_grid[0]}']")
        trans2_grid = [trans2_grid[0]]
        norm2_grid = [norm2_grid[0]]
    ...
    results = outer10_inner_holdout(
        ...
        norm1_grid=norm1_grid,
        norm2_grid=norm2_grid,
    ...
    if args.out_summary:
        ...
        with open(args.out_summary, "w") as f:
            ...
            f.write("fold\tthreshold\ttrans1\ttrans2\tnorm1\tnorm2\thidden_dim1\thidden_dim2\tlatent_dim\tlr\tbatch_size\tbeta\tzero_weight\tnonzero_weight\tinner_val_loss\tinner_val_metric\ttest_loss\ttest_metric\n")
            for i, (cfg, vl, vm, tl, tm) in enumerate(zip(
                ...
                f.write(f"{i}\t{cfg['threshold']}\t{cfg['trans1']}\t{cfg['trans2']}\t{cfg['norm1']}\t{cfg['norm2']}\t{cfg['hidden_dim1']}\t{cfg['hidden_dim2']}\t{cfg['latent_dim']}\t{cfg['lr']}\t{cfg['batch_size']}\t{cfg['beta']}\t{cfg['zero_weight']}\t{cfg['nonzero_weight']}\t{vl:.6f}\t{vm:.6f}\t{tl:.6f}\t{tm:.6f}\n")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="VAE with Weighted Reconstruction Loss and Multiple Evaluation Metrics")
